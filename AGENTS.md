...
- **ルール文書（AGENTS.md・スキル・docs）はコンパクトに保つ**。追記するときは既存との
  矛盾・重複を探して排除し、増やしたぶん減らせないか見直す
- 未完了タスク・要判断事項は `issues/` でファイル管理する
  （`NNN_タイトル.md`、frontmatter: title / created / status: open|closed / pr）
...
- **開発はブランチを切って PR を出し、スカッシュマージで main に取り込む**。
  スカッシュ後は PR タイトルがコミットメッセージになるため、PR タイトルは
  Conventional Commits 形式（`feat(keisan): ...` 等）にする。main への直 push は緊急時のみ
- PR を出すと CI が変更のあったアプリだけ check + build する（.github/workflows/ci.yml）
- UI・見た目に関わる変更は、**コミット前にローカルで動くものを見せて認識合わせする**
  （`task dev` / `task site`）。マージ・デプロイは OK が出てから
